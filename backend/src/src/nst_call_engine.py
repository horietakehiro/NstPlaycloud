...
from NstEngine.src.nst_utils import retreive_cmd_option4call_engine, load_config, automatically_decide_image_size
...
def request(content, style, options, config):

    engine = NstEngine(content.shape[1], content.shape[2] ,config)
    if options['log']:
    ...
    for step in tqdm.tqdm(range(int(options['step']))):
        ...
        transfer, loss = engine.fit(content=transfer, style=style, content_org=content)
        loss = loss[0]
        if options['log']:
...
def main(args=None):
    ...
    options = retreive_cmd_option4call_engine(args[1:])
    if options['auto']:
    ...
    transfer = request(content, style, options, config)
